...
{
    public sealed class CampaignAbilityObjectData
    {
        ...
        public CampaignAbilityObjectData(params ObjectModification[] modifications)
            : this()
        {
        ...
        internal CampaignAbilityObjectData()
        {
            _baseModifications = new Dictionary<int, ObjectModification>();
            _newModifications = new Dictionary<int, ObjectModification>();
        }

        public static CampaignAbilityObjectData Default => new CampaignAbilityObjectData() { _fileFormatVersion = ObjectDataFormatVersion.Normal, };
        ...
        public static CampaignAbilityObjectData Parse(Stream stream, bool leaveOpen = false)
        {
            try
            {
                var data = new CampaignAbilityObjectData();
                using (var reader = new BinaryReader(stream, new UTF8Encoding(false, true), leaveOpen))
                {
                    ...
                    {
                        var mod = ObjectModification.Parse(stream, true, true);
        ...
        public static void Serialize(CampaignAbilityObjectData objectData, Stream stream, bool leaveOpen = false)
        {
