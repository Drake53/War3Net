    ...
    public partial class JassScriptDecompiler
    {
        private bool TryDecompileWaitForConditionActionFunction(JassLoopStatementSyntax loopStatement, [NotNullWhen(true)] out TriggerFunction? actionFunction)
        {
            if (loopStatement.Statements.Length == 2 &&
                ...
                exitInvocationExpression.ArgumentList.Arguments.Items.IsEmpty &&
                ...
                callStatement.ArgumentList.Arguments.Items[0] is JassInvocationExpressionSyntax callInvocationExpression &&
                string.Equals(callInvocationExpression.IdentifierName.Token.Text, "RMaxBJ", StringComparison.Ordinal) &&
                callInvocationExpression.ArgumentList.Arguments.Items.Length == 2 &&
                callInvocationExpression.ArgumentList.Arguments.Items[0].TryGetIdentifierNameValue(out var variableName) &&
                ...
                exitFunctionDeclaration.IsConditionsFunction)
            {
                ...
                if (exitFunction.Statements.Length == 1 &&
                    ...
                    TryDecompileTriggerFunctionParameter(callInvocationExpression.ArgumentList.Arguments.Items[1], JassKeyword.Real, out var intervalParameter))
                {
